defmodule Ecto.Schema do
  ...
  * `__schema__(:redact_fields)` - Returns a list of redacted field names;
  ...
  @doc """
  Generates `:created_at` and `:updated_at` timestamp fields.
  ...
  ## Options
    ...
    * `:created_at` - the Ecto schema name of the field for insertion times or `false`
    * `:updated_at` - the Ecto schema name of the field for update times or `false`
    * `:created_at_source` - the name of the database column for insertion times or `false`
  ...
  **Important!** Please use this feature only when strictly necessary,
  ...
  def __define_timestamps__(mod, opts) do
    ...
    created_at = Keyword.get(timestamps, :created_at, :created_at)
    updated_at = Keyword.get(timestamps, :updated_at, :updated_at)

    if created_at do
      opts = if source = timestamps[:created_at_source], do: [source: source], else: []
      Ecto.Schema.__field__(mod, created_at, type, opts)
    end
    ...
    with [_ | _] = fields <- Enum.filter([created_at, updated_at], & &1) do
