defmodule Ecto.Integration.IntervalTest do
  ...
  @posted ~D[2014-01-01]
  @created_at ~N[2014-01-01 02:00:00]

  setup do
    TestRepo.insert!(%Post{posted: @posted, created_at: @created_at})
    :ok
  end
  ...
  test "datetime_add with year" do
    ...
    assert [~N[2015-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 1, "year"))
    assert [~N[2015-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 1.0, "year"))
    assert [~N[2015-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^1, "year"))
    assert [~N[2015-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^1.0, "year"))
    assert [~N[2015-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "year"))
  end

  test "datetime_add with month" do
    dec = Decimal.new(3)
    assert [~N[2014-04-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 3, "month"))
    assert [~N[2014-04-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 3.0, "month"))
    assert [~N[2014-04-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^3, "month"))
    assert [~N[2014-04-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^3.0, "month"))
    assert [~N[2014-04-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "month"))
  end

  test "datetime_add with week" do
    dec = Decimal.new(3)
    assert [~N[2014-01-22 02:00:00]] =
            TestRepo.all(from p in Post, select: datetime_add(p.created_at, 3, "week"))
    assert [~N[2014-01-22 02:00:00]] =
            TestRepo.all(from p in Post, select: datetime_add(p.created_at, 3.0, "week"))
    assert [~N[2014-01-22 02:00:00]] =
            TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^3, "week"))
    assert [~N[2014-01-22 02:00:00]] =
            TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^3.0, "week"))
    assert [~N[2014-01-22 02:00:00]] =
            TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "week"))
  end

  test "datetime_add with day" do
    dec = Decimal.new(5)
    assert [~N[2014-01-06 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 5, "day"))
    assert [~N[2014-01-06 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 5.0, "day"))
    assert [~N[2014-01-06 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^5, "day"))
    assert [~N[2014-01-06 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^5.0, "day"))
    assert [~N[2014-01-06 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "day"))
  end

  test "datetime_add with hour" do
    dec = Decimal.new(60)
    assert [~N[2014-01-03 14:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 60, "hour"))
    assert [~N[2014-01-03 14:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 60.0, "hour"))
    assert [~N[2014-01-03 14:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^60, "hour"))
    assert [~N[2014-01-03 14:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^60.0, "hour"))
    assert [~N[2014-01-03 14:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "hour"))
  end

  test "datetime_add with minute" do
    dec = Decimal.new(90)
    assert [~N[2014-01-01 03:30:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 90, "minute"))
    assert [~N[2014-01-01 03:30:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 90.0, "minute"))
    assert [~N[2014-01-01 03:30:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^90, "minute"))
    assert [~N[2014-01-01 03:30:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^90.0, "minute"))
    assert [~N[2014-01-01 03:30:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "minute"))
  end

  test "datetime_add with second" do
    dec = Decimal.new(90)
    assert [~N[2014-01-01 02:01:30]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 90, "second"))
    assert [~N[2014-01-01 02:01:30]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 90.0, "second"))
    assert [~N[2014-01-01 02:01:30]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^90, "second"))
    assert [~N[2014-01-01 02:01:30]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^90.0, "second"))
    assert [~N[2014-01-01 02:01:30]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "second"))
  end
  ...
  test "datetime_add with millisecond" do
    ...
    assert [~N[2014-01-01 02:00:01]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 1500, "millisecond"))
    assert [~N[2014-01-01 02:00:01]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 1500.0, "millisecond"))
    assert [~N[2014-01-01 02:00:01]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^1500, "millisecond"))
    assert [~N[2014-01-01 02:00:01]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^1500.0, "millisecond"))
    assert [~N[2014-01-01 02:00:01]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "millisecond"))
  end
  ...
  test "datetime_add with microsecond" do
    ...
    assert [~N[2014-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 1500, "microsecond"))
    assert [~N[2014-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, 1500.0, "microsecond"))
    assert [~N[2014-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^1500, "microsecond"))
    assert [~N[2014-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^1500.0, "microsecond"))
    assert [~N[2014-01-01 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(p.created_at, ^dec, "microsecond"))
  end

  test "datetime_add with dynamic" do
    created_at = @created_at
    assert [~N[2015-01-01 02:00:00]]  =
           TestRepo.all(from p in Post, select: datetime_add(^created_at, ^1, ^"year"))
    assert [~N[2014-04-01 02:00:00]]  =
           TestRepo.all(from p in Post, select: datetime_add(^created_at, ^3, ^"month"))
    assert [~N[2014-01-22 02:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(^created_at, ^3, ^"week"))
    assert [~N[2014-01-06 02:00:00]]  =
           TestRepo.all(from p in Post, select: datetime_add(^created_at, ^5, ^"day"))
    assert [~N[2014-01-03 14:00:00]] =
           TestRepo.all(from p in Post, select: datetime_add(^created_at, ^60, ^"hour"))
    assert [~N[2014-01-01 03:30:00]] =
           TestRepo.all(from p in Post, select: datetime_add(^created_at, ^90, ^"minute"))
    assert [~N[2014-01-01 02:01:30]] =
           TestRepo.all(from p in Post, select: datetime_add(^created_at, ^90, ^"second"))
  end
  ...
  test "datetime_add with dynamic in filters" do
    ...
    field = :created_at
    assert [_]  =
           TestRepo.all(from p in Post, where: p.created_at > datetime_add(^created_at, ^-1, "year"))
    assert [_]  =
           TestRepo.all(from p in Post, where: p.created_at > datetime_add(^created_at, -3, "month"))
    assert [_]  =
           TestRepo.all(from p in Post, where: field(p, ^field) > datetime_add(^created_at, ^-3, ^"week"))
    assert [_]  =
           TestRepo.all(from p in Post, where: field(p, ^field) > datetime_add(^created_at, -5, ^"day"))
  end
  ...
  test "datetime_add with utc_datetime" do
    {:ok, datetime} = DateTime.from_naive(@created_at, "Etc/UTC")
    TestRepo.insert!(%User{created_at: datetime})
    ...
    assert [^datetime] =
           TestRepo.all(from p in User, select: datetime_add(p.created_at, 1, "year"))
    assert [^datetime] =
           TestRepo.all(from p in User, select: datetime_add(p.created_at, 1.0, "year"))
    assert [^datetime] =
           TestRepo.all(from p in User, select: datetime_add(p.created_at, ^1, "year"))
    assert [^datetime] =
           TestRepo.all(from p in User, select: datetime_add(p.created_at, ^1.0, "year"))
    assert [^datetime] =
           TestRepo.all(from p in User, select: datetime_add(p.created_at, ^dec, "year"))
  end
