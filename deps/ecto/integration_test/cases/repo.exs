defmodule Ecto.Integration.RepoTest do
  ...
  test "insert and fetch a schema with utc timestamps" do
    ...
    assert [%{created_at: ^datetime}] = TestRepo.all(User)
  end
  ...
  test "update all with casting and dumping" do
    ...
    assert {1, nil} = TestRepo.update_all(Post, set: [visits: visits, created_at: datetime])
    assert %Post{visits: 13, created_at: ^datetime} = TestRepo.get(Post, id)
  end
