defmodule Ecto.Integration.TypeTest do
  ...
  test "primitive types" do
    ...
    TestRepo.insert!(%Post{blob: blob, public: true, visits: integer, uuid: uuid,
                           counter: integer, created_at: datetime, intensity: float})
    ...
    # NaiveDatetime
    assert [^datetime] = TestRepo.all(from p in Post, where: p.created_at == ^datetime, select: p.created_at)

    # Datetime
    datetime = DateTime.from_unix!(System.os_time(:second), :second)
    TestRepo.insert!(%User{created_at: datetime})
    assert [^datetime] = TestRepo.all(from u in User, where: u.created_at == ^datetime, select: u.created_at)
  ...
  test "aggregate types" do
    datetime = ~N[2014-01-16 20:26:51]
    TestRepo.insert!(%Post{created_at: datetime})
    query = from p in Post, select: max(p.created_at)
  ...
  test "aggregate filter types" do
    datetime = ~N[2014-01-16 20:26:51]
    TestRepo.insert!(%Post{created_at: datetime})
    query = from p in Post, select: filter(max(p.created_at), p.public == ^true)
    assert [^datetime] = TestRepo.all(query)
  end
  ...
  test "schemaless calendar types" do
    datetime = ~N[2014-01-16 20:26:51]
    assert {1, _} =
           TestRepo.insert_all("posts", [[created_at: datetime]])
    assert {1, _} =
           TestRepo.update_all("posts", set: [created_at: datetime])
    assert [_] =
           TestRepo.all(from p in "posts", where: p.created_at >= ^datetime, select: p.created_at)
    assert [_] =
           TestRepo.all(from p in "posts", where: p.created_at in [^datetime], select: p.created_at)
    assert [_] =
           TestRepo.all(from p in "posts", where: p.created_at in ^[datetime], select: p.created_at)
  end
end
