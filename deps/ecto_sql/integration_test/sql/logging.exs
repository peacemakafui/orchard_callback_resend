  ...
  describe "parameter logging" do
    ...
    test "for insert_all with entries and placeholders" do
      ...
      row1 = [
        int: int,
        uuid: uuid_query,
        created_at: {:placeholder, :datetime},
      ...
      row2 = [
        int: int_query,
        uuid: uuid2,
        created_at: {:placeholder, :datetime2},
    ...
    test "for insert" do
      ...
      param_regex =
        ~r/\[(?<int>.+), \"(?<uuid>.+)\", (?<created_at>.+), (?<updated_at>.+), \"(?<bid>.+)\"\]/
      ...
      assert param_logs["created_at"] =~ @naive_datetime_regex
    ...
    test "for insert with conflict query" do
      ...
      param_regex =
        ~r/\[(?<int>.+), \"(?<uuid>.+)\", (?<created_at>.+), (?<updated_at>.+), \"(?<bid>.+)\", (?<conflict_update>.+), (?<conflict_int>.+), \"(?<conflict_uuid>.+)\"\]/
