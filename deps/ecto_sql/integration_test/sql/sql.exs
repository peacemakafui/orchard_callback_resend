defmodule Ecto.Integration.SQLTest do
  ...
  test "fragmented types" do
    ...
    query = from p in Post, where: fragment("? >= ?", p.created_at, ^datetime), select: p.created_at
  ...
  test "load" do
    created_at = ~N[2016-01-01 09:00:00]
    TestRepo.insert!(%Post{title: "title1", created_at: created_at, public: false})

    result = Ecto.Adapters.SQL.query!(TestRepo, "SELECT * FROM posts", [])
    posts = Enum.map(result.rows, &TestRepo.load(Post, {result.columns, &1}))
    assert [%Post{title: "title1", created_at: ^created_at, public: false}] = posts
  end
